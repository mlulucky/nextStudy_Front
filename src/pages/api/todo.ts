...
export const addAPI = async (token: string, data: ToDoCreateRequestDTO) => {
	const addResponse = await toDoHttpRequest(token).post(`/register`, data).catch((error)=>{console.error(error); null}); // request 요청 실패시 -> catch 문 실행, addResponse 에 null 할당
...
export const deleteAPI = async (token: string, todoId: number) => {
	try{
		const deleteResponse = await toDoHttpRequest(token).delete(`/${todoId}/delete`);
